...
function generarPeliculas() {
  ...
  document.getElementById('imagen2').src =
    'https://image.tmdb.org/t/p/w500/' + pelicula2.poster_path;
}
...
function verificarRespuesta(event) {
  if (banderaclicks) {
    if (event.target === boton1) {
      if (
        peliculas[currentIndex].vote_average >=
        peliculas[currentIndex === peliculas.length - 1 ? 0 : currentIndex + 1]
          .vote_average
      ) {
...
function verificarPutuacion() {
  if (nombreArchivo === 'facil.html') perder(30);
  else if (nombreArchivo === 'medio.html') perder(15);
  else if (nombreArchivo === 'dificil.html') perder(5);
}
function reiniciar() {
  if (record < a && RecordM) {
    record = a;
